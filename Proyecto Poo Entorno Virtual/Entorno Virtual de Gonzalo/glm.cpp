...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	strcat_s(filename, sizeof(name), name);
	free(dir);*/
	strcpy_s(dir, glmDirName(model->pathname));
	...
	nummaterials = 1;
	while (fscanf_s(file, "%s", buf, _countof(buf)) != EOF) {
		switch (buf[0]) {
		case '#':               /* comment */
			/* eat up rest of line */
		...
		case 'n':               /* newmtl */
			...
			char temp[128];
			strcpy_s(temp, buf);
			sscanf_s(temp, "%s %s", buf, _countof(buf));
			break;
		default:
			/* eat up rest of line */
			fgets(buf, sizeof(buf), file);
...
glmFirstPass(GLMmodel* model, FILE* file)
{
	...
	while (fscanf_s(file, "%s", buf, sizeof(buf)) != EOF) {
		...
		case 'm':
			...
			strcpy_s(temp, buf);
			sscanf_s(temp, "%s %s", buf, _countof(buf));
			//sscanf_s(buf, "%s %s", buf, sizeof(buf));
		...
		case 'f':               /* face */
			v = n = t = 0;
			fscanf_s(file, "%s", buf, _countof(buf));
...
glmReadPPM(char* filename, int* width, int* height)
{
	...
	fread_s(image, sizeof(image), sizeof(unsigned char), w * h * 3, fp);
	fclose(fp);
